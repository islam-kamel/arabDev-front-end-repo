  ...
  const submitHandler = async data => {
    try {
      await axios.post('http://localhost/api/v1/user/register', {
        ...
        date_of_birth: new Date(data.dateOfBirth).toISOString().slice(0, 10),
        email: data.email,
        password: data.password,
      })
    } catch (err) {
      userNameErrorRef.current.innerText = err?.response?.data?.username[0]
        ...
        <div className="form-div mt-6">
          ...
          <div>
            <div>
              ...
              <input name="password" required {...register('password')}></input>
